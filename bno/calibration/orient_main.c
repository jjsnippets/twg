 ...
 * physics and stores it in the DCD (FRS record 0x1F1F). Tare
 * corrects the ORIENTATION of the fused output frame and stores it
 * in the separate sensor-orientation record — the two are
 * independent, so taring never touches a calibration.
 *
 ...
 *     tare to flash so every later session — bno_app and the
 *     validation binaries included — boots in the tared frame.
 *   - Clear Tare (sh2_clearTare, --clear) drops the volatile tare. A
 *     tare previously persisted to flash is NOT erased: it reloads
 *     at the next reset and must be overwritten by a fresh tare +
 *     --persist.
 *
 * Usage:
 ...
 *
 * Must not run at the same time as bno_app, bno_cal, bno_cal_clear
 * or the validation binaries: one SPI HAL instance per process.
 *
...
/*
 * SH-2 euler convention (degrees), Tait-Bryan ZYX from the
 * rotation-vector quaternion: yaw about +Z, pitch about +Y, roll
 * about +X. Only the relative before/after behaviour and the
 * post-tare zero matter here, so the usual BNO085 display
...
static void printAttitude(const char *label, const OrientSample_t *s)
{
    ...
    printf("%s: yaw %.2f  pitch %.2f  roll %.2f deg  "
           "(rv accuracy %u, %s)\n",
           label, (double)yaw, (double)pitch, (double)roll,
...
static void printLiveLine(const OrientSample_t *s)
{
    float yaw, pitch, roll;
    eulerDeg(s, &yaw, &pitch, &roll);
    printf("  yaw %8.2f  pitch %7.2f  roll %7.2f deg  [rv acc %u]  \r",
...
static int doClear(void)
{
    ...
    if (!promptEnter("drop the volatile tare and return to the raw frame")) {
        goto abort;
    }
    ...
    sensor_orient_stop();
    printf("\nNOTE: a tare previously persisted to flash still reloads at "
           "the\nnext reset; overwrite it with a fresh bno_orient "
           "--persist.\n");
...
static int doOrient(bool allAxes, bool persist)
{
    ...
    /*
     * Mirror bno_app's flight policy (all dynamic calibration off,
     * fly on the saved DCD) so the heading shown — and the heading
     * the tare zeroes — is exactly what acquisition will see.
     */
    if (sh2_setCalConfig(0) != SH2_OK) {
        fprintf(stderr, "error: sh2_setCalConfig failed\n");
        sensor_orient_stop();
           ...
           "up to %u s)...\n", ACC_GOAL, SETTLE_TIMEOUT_S);
    {
        ...
        if (rc == 1) {
            fprintf(stderr,
                    "error: heading never settled to accuracy %d; run "
                    "bno_cal first\n", ACC_GOAL);
            sensor_orient_stop();
            return EXIT_ERROR;
        }
    ...
    if (!promptEnter("move the fixture to its swing-axis ZERO, keep it "
                     "stationary, then tare")) {
        goto abort;
    }
    ...
    if (!persist) {
        printf("\nNOTE: this tare is volatile — the next session open "
               "(HAL\nreset) reboots the chip and discards it. Run with "
               "--persist\nto store it in flash so bno_app and the "
               "validation\nbinaries start in the tared frame.\n");
        sensor_orient_stop();
        printf("\nRESULT: TARE APPLIED, VOLATILE (exit 0)\n");
        return EXIT_OK;
    }
    ...
    if (sh2_persistTare() != SH2_OK) {
        fprintf(stderr,
                "error: sh2_persistTare failed; the tare remains active "
    ...
    sensor_orient_stop();
    if (!sensor_orient_start()) {
        fprintf(stderr, "error: session reopen failed during "
                        "verification\n");
        return EXIT_ERROR;
    }
    if (sh2_setCalConfig(0) != SH2_OK) {
    ...
    sensor_orient_stop();
    printf("\nRESULT: TARE PERSISTED AND VERIFIED (exit 0)\n");
    return EXIT_OK;
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; ++i) {
        if (strcmp(argv[i], "--all") == 0) {
            ...
            fprintf(stderr, "usage: bno_orient [--all] [--persist] "
                            "[--clear] [--check]\n");
            return EXIT_ERROR;
        }
    }
