 ...
 * status byte) and the calibrated magnetic field vector the operator
...
typedef struct {
     ...
     * rotation vector, whose payload 'accuracy' field is something
     * else entirely (see rvErrRad below).
     *
     * Note on the gyro bit: it stays 0 whenever the gyro dynamic
     * calibration flag is disabled (the bno_app all-off policy). It
     * is only meaningful during the calibration flow itself, where
     * the flag is on.
     */
     ...
     * field the SH-2 calls 'accuracy' inside the RV report — it is
     * NOT the 0-3 calibrated scale. Diagnostic only.
     */
    float rvErrRad;
