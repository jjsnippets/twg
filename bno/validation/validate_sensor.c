/*
 * validate_sensor.c — SH-2 session owner for the validation binary.
 *
 ...
 * ImuValidateSample_t, which keeps a separate host/device timestamp pair
 * and sequence number per sensor group.
 *
 ...
 *
 * Build: make validate (Makefile links vendored sh2, app/sh2_hal_rpi.c, this file)
 */
...
bool sensor_validate_start(void)
{
    ...
    /*
     * Flight-time calibration policy: disable all dynamic calibration.
     * Mirrors bno_app so validation data reflects exactly how the production
     * app runs.
     */
